...
# From original paper Fig3, our group think that the last column, which are the outcome setting ratio alpha/beta = 1e-2, 
# emphasis more on matching the content of the photograph than the style of the artwork. Thus here, we choose default value 
# for content_weight (alpha) as 5, style_weight (beta) as 500. And the ratio is set to 1e-2 as default.
########################################################################################################################
CONTENT_WEIGHT = 5e0
...
STYLE_SCALE = 1.0
ITERATIONS = 500
VGG_PATH = './pretrained_VGG19/imagenet-vgg-verydeep-19.mat'
#POOLING = 'avg'
...
def imsave(path, img):
    """
    :input:
    :path: A path specified to save the image into.

    for pixel values larger than 255, force them to be 255
    for pixel values smaller than 0, force them to be 0.
    """
